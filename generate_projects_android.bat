...
@SET PM_Android_NDK_PATH=C:\AndroidNDK\android-ndk-r21d
@SET PM_PHYSXSDK_PATH=C:\git\physx4

:: Run packman to ensure dependencies are present and run cmake generation script afterwards
@call "%~dp0buildtools\get_build_deps.cmd" android
@if %ERRORLEVEL% neq 0 (
    @exit /b %errorlevel%
)

::Need linux libs, but windows tools, run us again to get the path
@call "%~dp0buildtools\packman5\packman.cmd" pull "%~dp0target_platform_deps.xml" --platform android --postscript "%~dp0buildtools\cmake_projects_android.bat"
@if %ERRORLEVEL% neq 0 (
    @exit /b %errorlevel%
) else (
    @echo Success!
